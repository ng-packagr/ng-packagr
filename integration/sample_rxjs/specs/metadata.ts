...
describe(`@sample/rxjs`, () => {
  ...
  describe(`rxjs.metadata.json`, () => {
    let METADATA;
    before(() => {
      METADATA = JSON.parse(fs.readFileSync(
        path.resolve(__dirname, '..', 'dist', 'rxjs.metadata.json'), 'utf-8'));
    });
    ...
    it(`should "importAs": "@sample/rxjs"`, () => {
      expect(METADATA['importAs']).to.equal('@sample/rxjs');
    });
