...
describe(`@sample/core`, () => {
  describe(`esm5/sample-core.js.map`, () => {
    let sourceMap;
    before(() => {
      sourceMap = fs.readJsonSync(
        path.resolve(__dirname, '../dist/esm5/sample-core.js.map')
      );
    });
    ...
    it(`should have 'sources' and 'sourcesContent' property`, () => {
      ...
      expect(sourceMap.sources).to.have.lengthOf(
        sourceMap.sourcesContent.length
      );
    });

    it(`should reference each 'sources' path with a common prefix`, () => {
      const everyUeveryMe = (sourceMap.sources as string[]).every(fileName =>
        fileName.startsWith(`ng://@sample/core`)
      );
