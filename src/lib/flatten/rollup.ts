...
export async function rollupBundleFile(opts: RollupOptions): Promise<void[]> {
  ...
  const bundle = await rollup.rollup({
    ...
    input: opts.entry,
    plugins: [nodeResolve(), commonJs(), sourcemaps(), { transform: opts.transform }, { transform: opts.transform }],
    onwarn: warning => {
  ...
  result.map.sources = result.map.sources.map(sourcePath => {
    ...
    if (sourcePath && sourcePath.indexOf(opts.sourceRoot.replace('//', '/')) > 0) {
      return sourcePath.substr(sourcePath.indexOf(opts.sourceRoot));
    }
