...
export const processAssets: BuildStep = async ({ artefacts, entryPoint, pkg }): Promise<NgArtefacts> => {
  // process templates
  const templates = await Promise.all(
    artefacts.templates().map(async template => {
      return {
        name: template,
  ...
  const stylesheets = await Promise.all(
    artefacts.stylesheets().map(async stylesheet => {
      return {
        name: stylesheet,
 ...
 */
const processTemplate = (templateFilePath: string): Promise<string> =>
