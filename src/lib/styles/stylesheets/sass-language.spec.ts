...
describe('sass-language', () => {
  ...
  describe('getPackageScope', () => {
    ...
    it('should use the enclosing package root for a stylesheet within node_modules', () => {
      ...
      expect(scope('/app/node_modules/foo-node_modules/a.scss')).toBe('/app/node_modules/foo-node_modules');
    ...
    it('should use the innermost package root for a nested dependency', () => {
      expect(scope('/app/node_modules/dep/node_modules/pkg/a.scss')).toBe('/app/node_modules/dep/node_modules/pkg');
    ...
    it('should support Windows path separators', () => {
      expect(getPackageScope('C:\\app\\node_modules\\pkg\\sub\\a.scss', 'C:\\app')).toBe('C:/app/node_modules/pkg');
  ...
  describe('package resolution caching', () => {
    ...
    beforeAll(async () => {
      ...
      // A scoped dependency with its own nested version of `theme`.
      await writePackage(join(projectRoot, 'node_modules', '@scope', 'dependency', 'node_modules', 'theme'), 'scoped');
    ...
    it('should share a package resolution between the subdirectories of a dependency', async () => {
      const dependencyRoot = join(projectRoot, 'node_modules', 'dependency');
    ...
    it('should share a package resolution within a scoped dependency but not across its scope', async () => {
      const scopeRoot = join(projectRoot, 'node_modules', '@scope');
      const first = await compile(join(scopeRoot, 'dependency', 'sub1', 'styles.scss'));
      const second = await compile(join(scopeRoot, 'dependency', 'sub2', 'styles.scss'));
      // Another package of the same scope, which has no nested version of `theme`.
      const other = await compile(join(scopeRoot, 'other', 'styles.scss'));

      expect(first).toContain('content: "scoped";');
      expect(second).toContain('content: "scoped";');
      expect(other).toContain('content: "project";');
      expect(resolveRequests.length).toBe(2);
    });
